...
int main(int argc, char **argv)
{
    ...
    ++iter_image, ++my_progress_bar )
  {
    ...
    // If at this point focal is still equal to -1.0, then it means that the camera sensor is not part of the sensor_width_camera_database.txt
    // and that the user did not give the focal length as input -> Solution: focal length can be approximated as 1.2*max(w,h)
    // The line below was added after the issue discussed in here: https://github.com/openMVG/openMVG/issues/477   
    if (focal < 0) focal = 1.2*std::max(width,height);
